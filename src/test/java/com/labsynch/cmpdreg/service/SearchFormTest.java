...
public class SearchFormTest {
   ...
   // @Test
    public void basicStructureSearch(){
		...
		MolHandler mh = null;
		try {
//			mh = new MolHandler(parent.getMolStructure());
			mh = new MolHandler("C1=CC=CC=C1");

		} catch (MolFormatException e) {
			System.out.println("bad structure error: " + parent.getMolStructure());
		}

		Molecule mol = mh.getMolecule();
		System.out.println("search with structure: " + mol.toFormat("smiles"));
		String searchTerm = "SUBSTRUCTURE";
		float similarity = 0.0f;
		int[] parentHits = structureService.searchMolStructures(mol.toFormat("mol"), "Parent_Structure", "Parent",searchTerm, similarity);
    ...
    @Test
    public void basicStructureSearch_2(){
    	    	
		MolHandler mh = null;
		try {
			mh = new MolHandler("COC1=CC(Cl)=CC=C1");
		} catch (MolFormatException e) {
			System.out.println("bad structure error: " );
		}
